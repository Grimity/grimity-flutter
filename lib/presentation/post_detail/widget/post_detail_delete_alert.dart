...
Future<void> showDeletePostAlert(Post post, BuildContext context, WidgetRef ref) {
  ...
  final alert = GdsAlert(
    ...
    onPrimaryTap: () async {
      context.pop();
      final result = await ref.read(postDetailDataProvider(post.id).notifier).deletePost(post.id);
